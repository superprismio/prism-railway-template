# Change Request Flow

This document shows the current Prism change-request flow as visual diagrams.

It focuses on:

- board state changes
- Codex execution flow
- GitHub branch and PR flow
- Railway shared staging and PR preview paths

## At A Glance

```mermaid
flowchart TD
    A[New CR created] --> B[submitted / Inbox]
    B --> C[Codex triage pass]
    C --> D[ready-for-agent]
    D --> E[Admin review]
    E --> F[Continue agent]
    F --> G[in-progress]
    G --> H[Codex edits target repo]
    H --> I[Commit on CR branch]
    I --> J[Push branch to GitHub]
    J --> K[Deploy preview]
    K --> L[awaiting-review]
    L --> M{Needs more work?}
    M -- Yes --> N[changes-requested]
    N --> F
    M -- No --> O[Human PR / merge flow]
    O --> P[approved / closed]
```

## Board State Machine

```mermaid
stateDiagram-v2
    [*] --> submitted
    submitted --> triaging: Codex triage starts
    triaging --> ready-for-agent: Triage complete
    ready-for-agent --> in-progress: Admin continues run
    in-progress --> awaiting-review: Run completes
    in-progress --> ready-for-agent: Run fails / retry needed
    awaiting-review --> changes-requested: Review asks for changes
    changes-requested --> in-progress: Admin continues same CR branch
    awaiting-review --> approved: Human accepts result
    approved --> closed
    rejected --> closed
```

## Runtime Sequence

```mermaid
sequenceDiagram
    participant Admin as Admin UI / Discord
    participant Site as site
    participant API as api
    participant Runtime as codex-runtime
    participant Repo as Target Repo
    participant GH as GitHub
    participant Railway as Railway

    Admin->>Site: Create or continue CR
    Site->>API: POST /responses or PATCH request
    API->>API: Create execution record
    API->>Runtime: POST /v1/responses
    Runtime->>Repo: Clone or reuse CR workspace
    Runtime->>Repo: Checkout CR branch from target base branch
    Runtime->>Runtime: Run Codex with request context
    Runtime->>Repo: Commit workspace changes
    Runtime->>GH: Push CR branch
    Runtime->>Railway: Deploy shared staging target when configured
    Runtime-->>API: responseText + branch + commit + trace + deploy info
    API->>API: Update execution and CR status
    API-->>Site: Updated board data
    Site-->>Admin: Review status, links, logs
```

## Branch And PR Flow

Current working rule:

- new CRs should branch from the configured target environment branch first
- for `daohaus-admin` staging that branch is currently `develop`
- existing CRs resume from their own branch first

```mermaid
flowchart LR
    A[Target base branch: develop] --> B[codex/cr-<number>-<slug>]
    B --> C[Codex commits]
    C --> D[Push to GitHub origin]
    D --> E[Review from branch]
    E --> F[Open PR into develop]
    F --> G[Railway PR environment]
    G --> H[Human review]
    H --> I[Merge to develop]
    I --> J[Later promote develop to production branch]
```

## Preview Paths

There are two preview modes in play right now.

### 1. Shared staging fallback

```mermaid
flowchart TD
    A[CR run completes] --> B[Deploy to agent-target-staging]
    B --> C[Shared preview URL]
    C --> D[Board stores deploy URL]
    D --> E[Admin reviews in shared staging]
```

Shared staging notes:

- service: configurable per deployment
- current URL: configured in the target environment record
- this is the short-term fallback preview target when Railway PR environments are not enabled

### 2. PR environment path

```mermaid
flowchart TD
    A[CR branch pushed] --> B[Human opens PR to develop]
    B --> C[Railway PR environment created]
    C --> D[Preview URL generated by Railway]
    D --> E[Review happens on PR preview]
    E --> F[PR merged or closed]
    F --> G[Railway PR environment removed]
```

PR environment notes:

- Railway PR environments are project-scoped
- they are created when a GitHub PR is opened
- they are not created from branch existence alone

## Failure And Retry Flow

```mermaid
flowchart TD
    A[in-progress execution] --> B{Run succeeded?}
    B -- No --> C[Execution marked failed]
    C --> D[CR moves back to ready-for-agent]
    D --> E[Admin reviews logs and comments]
    E --> F[Continue agent]
    F --> A
    B -- Yes --> G[Execution completed]
    G --> H[CR moves to awaiting-review]
```

Typical failure classes:

- transport failure between `api` and `codex-runtime`
- target repo fetch or branch prep failure
- build or validation failure in target repo
- deploy failure in Railway

## Data Objects

```mermaid
erDiagram
    CHANGE_REQUEST ||--o{ CHANGE_REQUEST_EXECUTION : has
    CHANGE_REQUEST }o--|| TARGET_APP : targets
    CHANGE_REQUEST }o--|| TARGET_ENVIRONMENT : runs_in
    CHANGE_REQUEST ||--o{ AGENT_SESSION : links
    AGENT_SESSION ||--o{ AGENT_MESSAGE : contains

    CHANGE_REQUEST {
      string id
      int requestNumber
      string status
      string title
      string targetAppId
      string targetEnvironmentId
    }

    CHANGE_REQUEST_EXECUTION {
      string id
      string status
      string branchName
      string commitSha
      string deployUrl
      json meta
    }

    TARGET_APP {
      string id
      string slug
      string repoUrl
      string defaultBranch
    }

    TARGET_ENVIRONMENT {
      string id
      string slug
      string branch
      string baseUrl
      json deployConfig
    }
```

## Current Review Surface

Today the board should surface:

- CR status
- latest execution summary
- runtime trace
- GitHub branch link
- GitHub compare / PR link
- shared staging preview link

Longer term it should also track:

- PR number and PR URL
- Railway PR environment URL
- PR review state and requested-changes feedback
