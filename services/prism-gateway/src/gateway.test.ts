...
test("gateway stores, leases, audits, rotates, and revokes trusted credentials", async () => {
  ...
  try {
    ...
    const pending = await jsonRequest(baseUrl, "/connections", {
      method: "POST",
      headers: { "content-type": "application/json", "x-gateway-token": siteToken },
      body: JSON.stringify({ key: "pending-github", provider: "github", label: "Pending GitHub",
        authType: "api-key", credentials: {}, envBindings: { GH_TOKEN: "token" } }),
    });
    assert.equal(pending.response.status, 201);
    const catalog = await jsonRequest(baseUrl, "/credential-bundles", {
      headers: { "x-gateway-token": siteToken },
    });
    assert.equal((catalog.body.credentials as Array<{key: string}>).some(x => x.key === "pending-github"), false);
    assert.equal((catalog.body.credentials as unknown[]).length, 26);
    const inventory = await jsonRequest(baseUrl, "/connections", {
      headers: { "x-gateway-token": siteToken },
    });
    assert.equal((inventory.body.connections as Array<{key: string}>).some(x => x.key === "pending-github"), true);
    const explicitPending = await jsonRequest(baseUrl, "/credential-bundles/lease", {
      method: "POST",
      headers: { "content-type": "application/json", "x-gateway-token": codexToken },
      body: JSON.stringify({ credentials: ["pending-github"] }),
    });
    assert.equal(explicitPending.response.status, 409);
    assert.equal(explicitPending.text.includes("CREDENTIAL_LEASE_SECRET_MISSING"), true);
    await jsonRequest(baseUrl, `/connections/${(pending.body.connection as {id: string}).id}/credentials`, {
      method: "PUT",
      headers: { "content-type": "application/json", "x-gateway-token": siteToken },
      body: JSON.stringify({ credentials: { token: "configured-test-token" } }),
    });
    const readyCatalog = await jsonRequest(baseUrl, "/credential-bundles", {
      headers: { "x-gateway-token": siteToken },
    });
    assert.equal((readyCatalog.body.credentials as Array<{key: string}>).some(x => x.key === "pending-github"), true);
