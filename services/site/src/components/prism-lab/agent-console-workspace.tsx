...
export function AgentConsoleWorkspace({ profile: initialProfile, activity, sessions, canManageSettings }: { profile: AgentProfileRecord; activity: AgentProfileActivityItem[]; sessions: AgentProfileSessionSummary[]; canManageSettings: boolean }) {
  ...
  async function uploadAvatar(formData: FormData) {
    ...
    try {
      ...
      const response = await fetch(`/admin/agent-profiles/${encodeURIComponent(profile.key)}/avatar`, { method: "POST", body: payloadBody })
      const payload = await response.json().catch(() => null) as { profile?: AgentProfileRecord; error?: string } | null
      if (!response.ok || !payload?.profile) throw new Error(payload?.error || "Could not upload agent avatar")
      setProfile(payload.profile)
      setNotice(`Avatar uploaded and saved as profile version ${payload.profile.version}.`)
      router.refresh()
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Could not upload agent avatar")
    } finally {
