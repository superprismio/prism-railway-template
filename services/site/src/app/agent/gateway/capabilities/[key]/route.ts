...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  try {
    return NextResponse.json(await prismGatewayRequest(`/capabilities/${encodeURIComponent(key)}`, {
      method: "PATCH",
      body: JSON.stringify(body),
    }));
  } catch (error) {
    const status = error instanceof PrismGatewayError ? error.status : 500;
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "CAPABILITY_UPDATE_FAILED" }, { status });
  }
}
