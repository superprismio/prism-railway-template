...
export async function GET(request: Request, context: RouteContext) {
  ...
  try {
    const format = transcriptFormat(request);
    const transcript = await readCaptureTranscriptFile(id, format);
    if (format === "json") {
      return NextResponse.json({
        ok: true,
        captureId: id,
        content: JSON.parse(transcript.content.toString("utf8")) as unknown,
      });
    }
