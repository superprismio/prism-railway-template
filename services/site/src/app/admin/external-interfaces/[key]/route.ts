...
import { deleteExternalInterface, getExternalInterface, listInteractionAccessEvents, resolveExternalInterface, upsertExternalInterface } from '@/lib/app-core';
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const key = readRouteParam((await context.params).key);
  const resolved = resolveExternalInterface(key);
  return resolved
    ? NextResponse.json({ ok: true, resolved, recentEvents: listInteractionAccessEvents(key, 100) })
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  try {
    return NextResponse.json({ ok: true, interface: upsertExternalInterface(externalInterfaceInput(await request.json().catch(() => null), existing)!) });
  } catch (error) {
