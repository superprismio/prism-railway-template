...
import { getAgentProfile, listAgentProfileActivity, listAgentProfileSessions, normalizeAgentProfileBindingPolicy } from "@/lib/app-core"
  ...
  return (
    <div className="px-4 py-5 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl">
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1.25fr_0.75fr]">
        ...
        <section><h2 className="font-semibold">Channels and interfaces</h2><p className="mt-1 text-xs text-muted-foreground">Console access is full. Each external surface resolves to this agent with its own narrowing policy.</p><div className="mt-3 divide-y divide-border/60 border border-border/60"><div className="p-3 text-sm"><div className="flex justify-between gap-3"><span className="font-medium">Prism Console</span><Badge>full</Badge></div><div className="mt-1 text-xs text-muted-foreground">Built in · authenticated operator surface</div></div>{profile.bindings.map((binding) => { const policy = normalizeAgentProfileBindingPolicy(binding.configuration); return <div key={binding.id} className="p-3 text-sm"><div className="flex justify-between gap-3"><span className="font-medium">{binding.label || binding.surfaceKey}</span><div className="flex gap-2"><Badge variant="outline">{binding.surfaceType}</Badge><Badge variant="outline">{policy.accessMode}</Badge></div></div><div className="mt-1 font-mono text-xs text-muted-foreground">{binding.surfaceKey}</div><div className="mt-1 text-xs text-muted-foreground">{policy.rateLimit.maxRequests} requests / {policy.rateLimit.windowSeconds}s{policy.allowedWorkflows.length ? ` · ${policy.allowedWorkflows.length} allowed workflows` : ""}</div></div> })}</div>{workspace.data.session.capabilities.includes("canManageSettings") && !profile.systemKey ? <div className="mt-3"><AgentBindingForm profileKey={profile.key} /></div> : null}</section>
      </div>
    </div></div>
  )
}
